          ...
          {stats.map((stat, index) => (
            <div
              ...
              <LiquidGlass
                ...
                <stat.icon
                  className={`w-6 h-6 ${
                    stat.label === "Total Triggers" && "text-[#FF6B00]"
                  } ${
                    stat.label === "Email Notifications" && "text-[#00A656]"
                  } ${
                    stat.label === "Push Notifications" && "text-[#7F5FFF]"
                  } ${stat.label === "SMS Notifications" && "text-[#2A85FF]"}`}
                />
              </LiquidGlass>
              ...
              <div className="flex items-center gap-2">
                ...
                {stat.label === "Total Triggers" && (
                  <span className="text-text-secondary text-sm">vs last year</span>
                )}
                {stat.label === "Email Notifications" && (
                  <span className="text-text-secondary text-sm">
                    Requires attention
                  </span>
                )}
                {stat.label === "SMS Notifications" && (
                  <span className="text-text-secondary text-sm">Need to assign</span>
                )}
                {stat.label === "Push Notifications" && (
                  <span className="text-text-secondary text-sm">
                    Lead to Customer
                  </span>
                )}
