...
const TeamReportModal = ({ teams }: { teams: TTeam[] }) => {
  ...
  return (
    <div className="flex items-center justify-center p-2">
      <Select
        onValueChange={(value) => {
          vieTeamReportHandler(value);
        }}
      >
        <SelectTrigger className="w-[200px]">
          <SelectValue placeholder="View team Report" />
        </SelectTrigger>

        <SelectContent>
          <SelectItem value="All">Select A team</SelectItem>
          {teams.map((team) => (
            <SelectItem
              key={team?.leader?.id}
