...
const LeaderDataTable = ({ members, setModalOpen, setSelectedWorkers }: MemberProps) => {
            ...
            <Table>
                ...
                <TableBody>
                    {members.map((member) => {
                        ...
                        const acceptedLeads = member.currentBatch.assignedCustomers.length;
                        ...
                        return (
                            <TableRow key={member.id}>
                                ...
                                <TableCell className="flex gap-2 items-center">
                                    ...
                                    />
                                    <Plus className="h-4 w-4 cursor-pointer" onClick={() => {
                                        setModalOpen(true);
                ...
                <DialogContent>
                    ...
                    {selectedMember && (
                        <div className="space-y-2 text-sm">
                            <p><strong>Name:</strong> {selectedMember.agent?.name}</p>
                            <p><strong>Email:</strong> {selectedMember.agent?.email}</p>
                            <p><strong>Phone:</strong> {selectedMember.agent?.phone}</p>
                            <p><strong>Assigned Leads:</strong> {selectedMember.currentBatch.totalCustomers}</p>
                            <p><strong>Accepted Leads:</strong> {selectedMember.currentBatch.assignedCustomers.length}</p>
                            <p><strong>Pending Leads:</strong> {selectedMember.currentBatch.pendingCustomers.length}</p>
                        </div>
                    )}
                </DialogContent>
            </Dialog>
