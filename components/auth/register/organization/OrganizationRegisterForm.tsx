...
import {
  calculatePrice,
  validateCouponCode,
} from "@/service/coupon/couonService";
...
export default function OrganizationRegisterForm({
  ...
  planData?: { planId: string; slug: string };
}) {
  const [plans, setPlans] = useState<TPlan[]>([]);
  const [couponValidationResult, setCouponValidationResult] =
    useState<any>(null);
  ...
  const onFormSubmit = async (data: OrgFormValues) => {
    ...
    try {
      ...
      if (!orgRes.success) {
        toast.error(orgRes.message || "Failed to create organization", {
          id: toastId,
        });
        return;
      }
      ...
      toast.loading("Organization created! Processing payment...", {
        id: toastId,
      });
      ...
      if (purchaseRes.success) {
        ...
        toast.error(
          `Organization created but payment failed: ${purchaseRes.message}`,
          {
            id: toastId,
          },
        );
              ...
              render={({ field }) => (
                <FormItem className="col-span-2">
                  ...
                  {couponValidationResult && (
                    <p
                      className={`mt-1 text-sm ${couponValidationResult.valid ? "text-green-600" : "text-red-600"}`}
                    >
              ...
              <Card>
                <CardContent className="pt-6">
                  {isLoadingPrice ? (
                    ...
                    <p className="text-center text-muted-foreground">
                      Select plan and billing cycle to see price
                    </p>
                  )}
                </CardContent>
              </Card>
